...
def calculate_adx(self, candles: List[dict], period: int = 14) -> float:
    ...
    if len(candles) < period * 2:
        return 0.0  # FIX: was 25.0 (always passed threshold)
    ...
    try:
        ...
        for tid, t in list(self.state.active_trades.items()):
            if tid not in open_ids and t.phase != TradePhase.CLOSED:
                ...
                # FIX: Try deals history FIRST (most accurate)
                try:
                ...
                # Fallback: estimate from SL/TP hit (not current price)
                if t.pnl == 0:
                    try:
                        ...
                        if close_price > 0 and t.lots > 0:
                            ...
                            log.warning(f"PnL estimated from price for {tid}: ${t.pnl:+.2f} (may be inaccurate)")
                    except Exception:
...
async def run(self):
    ...
    while self.state.running:
        try:
            ...
            log.warning(f"CancelledError #{consecutive_errors} (WebSocket disconnect)")
            ...
            if any(x in err for x in ["not connected", "not synchronized", "socket", "timed out", "unavailable"]):
                log.warning(f"Connection error #{consecutive_errors}: {e}")
                ...
                if consecutive_errors >= 10:
                    log.critical(f"Too many errors — reconnecting")
                    await self._reconnect()
                    consecutive_errors = 0
...
async def _reconnect(self):
    """Reconnect to MetaAPI — same logic as v4.0 bot."""
    log.warning("Attempting reconnect...")
    await self.tg.send("⚠️ <b>CONNECTION ISSUE</b> — reconnecting...")
    ...
    for attempt in range(3):
        try:
            ...
            if test and "balance" in test:
                self.state.balance = test.get("balance", self.state.balance)
                self.pos = PositionMgr(self.state, self.conn, self.db, self.tg)
    ...
    # All attempts failed — wait 2 min and try once more
    log.warning("All reconnects failed — waiting 2 min...")
    ...
    try:
        ...
        self.pos = PositionMgr(self.state, self.conn, self.db, self.tg)
        log.info("Reconnected after 2 min wait")
        await self.tg.send("✅ <b>RECONNECTED</b> (na 2 min)")
    except Exception as e:
        log.error(f"Final reconnect failed: {e}")
        await self.tg.send(f"❌ <b>RECONNECT FAILED</b> — watchdog will restart")
