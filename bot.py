“””
╔══════════════════════════════════════════════════════════════╗
║           XAUUSD GOLD SCALPER v1.5.2 (BUGFIX)                ║
║    ATR×2.5 | RR 1:2.0 | TP1 0.4R | 67% partial | 1% risk     ║
║                                                              ║
║  v1.5.2 fixes (vs v1.5.1):                                   ║
║   - FIX: duplicate rsi() method removed (second was          ║
║          shadowing first; broke MR signal)                   ║
║   - FIX: stoch_rsi() rewritten — was always returning 50.0   ║
║          due to broken recursive rsi() calls                 ║
║   - FIX: scalp_partial Telegram showed “50%” — now uses      ║
║          actual PARTIAL_PERCENT                              ║
║   - FIX: MR_RR_RATIO now actually applied for MR-dominant    ║
║          signals (was defined but ignored)                   ║
║   - FIX: lots formatted as 2 decimals in TG                  ║
║   - FIX: Database.save_trade now stores close_time           ║
║   - FIX: _cycle_count initialized in **init**                ║
║   - DOC:  MAX_SPREAD_POINTS clarified (it’s $ not points)    ║
║                                                              ║
║  Active signals (6): EMA, Sweep, OB, FVG, Momentum, MR,      ║
║                     RSI div, Double pattern, ADX bonus       ║
║  Disabled: Exhaustion, Round number (per v4 audit)           ║
╚══════════════════════════════════════════════════════════════╝
“””
...
# ─── Spread Filter ────────────────────────────────────────────
# NOTE: name says "POINTS" but XAUUSD spread is computed in $ (ask-bid).
# Value 3.5 = filter out spreads above $3.50 (typical ICM gold ≈ $0.30-$0.50).
...
MR_RR_RATIO: float = 1.5  # v1.5.2: now actually used
...
close_time: Optional[float] = None  # v1.5.2: track close time
pnl: float = 0.0
...
def save_trade(self, t: ScalpTrade, session: str = ""):
    # v1.5.2: store close_time when available
    close_iso = (
...
async def scalp_opened(self, t: ScalpTrade, session: str, confluence: int):
    ...
    await self.send(
        ...
        f"Lots: {t.lots:.2f} | RR: 1:{rr:.1f}\n"  # v1.5.2: 2-decimal lots
...
async def scalp_partial(self, t: ScalpTrade):
    # v1.5.2: use actual partial percent from config (was hardcoded "50%")
    pct = int(self.cfg.PARTIAL_PERCENT * 100)
    await self.send(f"✂️ <b>{pct}% CLOSED</b> @ ${t.tp1:.2f} → SL to BE", silent=True)
...
# ─── v1.5.2: SINGLE correct Wilder's RSI ─────────────────────
...
# ─── v1.5.2: helper for stoch_rsi (returns full RSI series) ──
def _rsi_series(self, candles: List[dict], period: int) -> List[float]:
    """Return a series of RSI values per candle (after warmup)."""
    if len(candles) < period + 1:
        return []
    closes = [c.get("close", 0) for c in candles]
    gains, losses = [], []
    ...
    rsi_values = [_rsi_from(avg_gain, avg_loss)]
    for i in range(period, len(gains)):
        avg_gain = (avg_gain * (period - 1) + gains[i]) / period
        avg_loss = (avg_loss * (period - 1) + losses[i]) / period
        rsi_values.append(_rsi_from(avg_gain, avg_loss))
    return rsi_values

# ─── v1.5.2: FIXED stoch_rsi (no more recursive rsi calls) ───
def stoch_rsi(self, candles: List[dict], rsi_period: int = 7,
              stoch_period: int = 7, k_smooth: int = 3) -> float:
    """Stochastic RSI %K, smoothed."""
    rsi_series = self._rsi_series(candles, rsi_period)
    ...
    for i in range(stoch_period - 1, len(rsi_series)):
        ...
        if rh == rl:
            ...
            k_raw.append((rsi_series[i] - rl) / (rh - rl) * 100)
    if not k_raw:
        return 50.0
    # SMA smoothing
    if len(k_raw) >= k_smooth:
        return sum(k_raw[-k_smooth:]) / k_smooth
    return k_raw[-1]
...
class ScalpSignal:
def **init**(self, state: BotState, analyzer: ScalpAnalyzer, session_mgr: SessionMgr):
self.state = state
self.az = analyzer
self.sm = session_mgr
self.cfg = state.cfg
...
def evaluate(self, price: float, spread: float) -> Optional[Tuple[Direction, float, float, float, int, str]]:
    ...
    mr_score_actual = 0  # v1.5.2: track for RR selection
    if mr_signal:
        mr_dir, mr_score_actual, mr_reason = mr_signal
    ...
    # ─── v1.5.2: MR-dominant signals use MR_RR_RATIO ─────────
    # MR is "dominant" when MR contributed >= half of the winning
    # direction's score (i.e. it's the main reason we're entering).
    winning_score = long_score if direction == Direction.LONG else short_score
    mr_dominant = (
        mr_signal is not None
        and mr_signal[0] == direction
        and self.cfg.MR_CONFLUENCE_SCORE >= winning_score / 2
    )
    if mr_dominant:
        rr = self.cfg.MR_RR_RATIO
        reasons.append("rr_mr")
    else:
    ...
    log.info(f"⚡ SIGNAL: {direction.value.upper()} | Score: {confluence} | RR: 1:{rr:.1f} | {reason_str}")
...
class PositionMgr:
def **init**(self, state: BotState, conn, db: Database, tg: Telegram):
self.state = state
self.conn = conn
self.db = db
self.tg = tg
    ...
    try:
        ...
        for tid, t in list(self.state.active_trades.items()):
            if tid not in open_ids and t.phase != TradePhase.CLOSED:
                ...
                t.close_time = time.time()  # v1.5.2: record close time

                # Try deals history FIRST (most accurate)
                try:
...
self.account = None
self.conn = None
self._cycle_count: int = 0  # v1.5.2: explicit init
...
async def connect(self):
    ...
    await self.tg.send(
        f"🤖 <b>Gold Scalper v1.5.2</b>\n"
        f"Balance: ${self.state.start_balance:.2f}\n"
        f"SL: ATR×2.5 | RR: 1:2.0 (MR 1:1.5) | TP1: 0.4R\n"
        f"Partial: {int(self.cfg.PARTIAL_PERCENT * 100)}% | Risk: {self.cfg.RISK_PERCENT}%\n"
